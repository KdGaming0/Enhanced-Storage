...
public interface ContainerEventHandler extends GuiEventListener {
	...
	default boolean mouseScrolled(final double x, final double y, final double scrollX, final double scrollY) {
		return this.getChildAt(x, y).filter(/* lambda$mouseScrolled$0 */ child -> child.mouseScrolled(x, y, scrollX, scrollY)).isPresent();
	}
	...
	private ComponentPath handleTabNavigation(final FocusNavigationEvent.TabNavigation tabNavigation) {
		...
		Collections.sort(sortedChildren, Comparator.comparingInt(/* lambda$handleTabNavigation$0 */ childx -> childx.getTabOrderGroup()));
	...
	private ComponentPath nextFocusPathInDirection(
		...
		Comparator<GuiEventListener> primaryComparator = Comparator.comparing(
			/* lambda$nextFocusPathInDirection$0 */ childx -> childx.getRectangle().getBoundInDirection(direction.getOpposite()), direction.coordinateValueComparator()
		);
		Comparator<GuiEventListener> secondaryComparator = Comparator.comparing(
			/* lambda$nextFocusPathInDirection$1 */ childx -> childx.getRectangle().getBoundInDirection(positiveDirectionOtherAxis.getOpposite()),
